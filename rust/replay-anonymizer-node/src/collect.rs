...
///
/// Memory sizing: collection peaks at ~2x the byte cap in Rust (collector vec + packed FFI
/// buffer), single-flight per pod (the pipeline anonymizes one message at a time), plus the packed
/// buffer retained until the produce settles. The dedicated producer's bounded queue sheds bursts
/// that outrun the broker, so the cap never compounds into producer-side backlog.
