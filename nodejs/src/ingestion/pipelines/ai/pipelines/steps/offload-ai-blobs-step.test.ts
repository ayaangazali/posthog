...
describe('offloadAiBlobsStep', () => {
    ...
    it('records a text-size forecast observation for text-only heavy props', async () => {
        ...
        const count = values.find((v) => v.metricName === 'llma_ai_blob_offload_text_bytes_count')?.value ?? 0
        const sum = values.find((v) => v.metricName === 'llma_ai_blob_offload_text_bytes_sum')?.value ?? 0
        expect(count).toBe(1)
