import './AIObservabilityTraceScene.scss'
...
function getQueueTracePath({
    ...
    return combineUrl(urls.aiObservabilityTrace(traceId), {
        ...sanitizeTraceUrlSearchParams(searchParams),
        back_to: 'reviews',
...
function TraceNavigation(): JSX.Element {
    ...
    const goToNewer = (): void => {
        if (newerTraceId) {
            router.actions.push(
                combineUrl(urls.aiObservabilityTrace(newerTraceId), {
                    ...baseSearchParams,
    ...
    const goToOlder = (): void => {
        if (olderTraceId) {
            router.actions.push(
                combineUrl(urls.aiObservabilityTrace(olderTraceId), {
                    ...baseSearchParams,
...
function TraceSceneWrapper(): JSX.Element {
    ...
    const backPath =
        backTo === 'generations'
            ? combineUrl(urls.aiObservabilityGenerations(), sanitizedBackSearchParams).url
            : backTo === 'reviews' && queueContext.isActive && queueContext.queueId && previousQueueTraceId && traceId
              ...
              : backTo === 'reviews'
                ...
                : combineUrl(urls.aiObservabilityTraces(), sanitizedBackSearchParams).url
    const forceBackTo =
        backTo === 'generations'
